...
// 이순신 시리즈 영화부제목 찍기
  ...
  showTxt('이순신 시리즈 영화 주요출연배우들:${
    actorList(actors.toList())}');

    // 다음 추가 이순신 시리즈가 있다면 어느배우를  추천하는가?
  showTxt('다음 이순신 시리즈가 있다면 추천배우는 공유다!');
  showTxt('공유의 취미는 ${
    recommActor['공유']?['취미']}다! 사는곳은 ${
      recommActor['공유']?['사는곳']}다! 공유의 자동차는 ${
        recommActor['공유']?['자동차']??"정보없음"}이다!');

  showTxt('다음 이순신 시리즈가 있다면 추천배우는 조인성이다!');
  showTxt('조인성의 취미는 ${
    recommActor['조인성']?['취미']}다! 사는곳은 ${
      recommActor['조인성']?['사는곳']}다! 조인성의 자동차는 ${
        recommActor['조인성']?['자동차']??"정보없음"}이다!');

    // [ 다트문법 널세이프티 적용사례!!! ]
    // -> Map형 데이터일 경우 내부 속성 불일치 등으로
    // null이 나올때 에러로 연결되므로 이런 경우에
    // 다트는 필수적으로 널 세이프티 처리를 강제한다!
    // 물음표(?)하나는 null일 경우 "null"문자열 출력대체
    // 물음표두개(??)는 null발생시 문자열 대체한 경우
    // 커스터마이징 대체문자열을 출력하게 해준다!

    // 익명함수를 만들어서 특정함수에 값으로 전달한다!!!
    japanShip((){print('개.박.살...! 일본배 침몰!');});

    // 람다함수를 호출하여 배의 숫자를 줄여보자!
...
const recommActor = {
  '조인성':
  {'나이':42, '하비':'날기', '사는곳':'아무데나', '자동차':'페라리'},
  '공유':
  {'나이':45, '취미':'비오게하기', '사는곳':'공유하우스'}
};
